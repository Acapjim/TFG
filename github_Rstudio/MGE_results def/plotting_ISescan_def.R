...
# Guardar el grÃ¡fico como un archivo SVG con un tamanyo adecuado
...
# Calcular la media y desviaciÃ³n estÃ¡ndar para la columna "total" de cada dataset
...
grafico_general <- ggplot(resumen, aes(x = variable, y = media, fill = grupo)) +
  ...
  labs(x = "Família", y = "IS per mostra", fill = "Grupo") +
  theme_minimal() + theme_base +
  scale_fill_manual(values = c("Cromosoma" = "#F8766D", 
                               "Plasmidis" = "#619CFF", 
                               "Genoma" = "#00BA38")) +
...
# Guardar el grafico como un archivo SVG con un tamaÃ±o adecuado
